...
const TransactionModalsDark = () => {
  ...
  const modalContent = {
    bill: { title: 'Add Bill', description: 'Enter bill details' },
    invoice: { title: 'Add Invoice', description: 'Create new invoice' },
    payment: { title: 'Add Payment', description: 'Record payment' },
    journalEntry: { title: 'Journal Entry', description: 'Create journal entry' }
  };

  const content = modalContent[activeModal] || { title: 'Transaction', description: '' };
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white rounded-lg p-6 w-full max-w-lg">
        ...
        <div className="bg-gray-50 rounded-lg p-4 mb-4">
          <p className="text-sm text-gray-600">Entity ID: {selectedEntity}</p>
          <p className="text-xs text-gray-400 mt-1">Full form implementation pending</p>
        </div>
        <div className="flex gap-2">
          <button onClick={closeModal} className="flex-1 bg-gray-200 text-gray-800 px-4 py-2 rounded hover:bg-gray-300">
            Cancel
          </button>
          <button onClick={closeModal} className="flex-1 bg-emerald-600 text-white px-4 py-2 rounded hover:bg-emerald-700">
            Save
          </button>
        </div>
      </div>
...
const AccountingSidebar = ({ entity }) => {
  ...
  const menuSections = [
    ...
    },
    {
      title: 'Settings',
      items: [
        { label: 'Accounting Settings', path: `/accounting/${entityId}/settings`, icon: Settings },
      ]
    },
  ];
  ...
  return (
    ...
    <aside className="w-56 bg-[#1a1a1a] border-r border-gray-800 flex flex-col flex-shrink-0 h-full">
      ...
      {/* Quick Actions */}
      <QuickActionsDark entityId={entityId} />
